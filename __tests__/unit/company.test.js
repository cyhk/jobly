...
describe("Test Company Class", () => {
  ...
  describe("create()", () => {
    test("should add company to database", async function () {
      const newCompany = {
        handle: "EARTH",
        name: "Planet Earth",
        employees: 13,
  ...
  describe("all()", () => {
    ...
    test("get all companies matching all search filters", async function () {
      const companies = await Company.all({
        search: "test",
        min_employees: 1,
        max_employees: 99,
    ...
    test("get all companies matching search filter", async function () {
    ...
    test("get no companies when search filter does not match", async function () {
      const companies = await Company.all({ search: "LKJS" });
      expect(companies).toEqual([]);
    });

    test("get all companies matching min_employees filter", async function () {
    ...
    test("get no companies when min_employees filter does not match", async function () {
      const companies = await Company.all({ min_employees: 88 });
      expect(companies).toEqual([]);
    });

    test("get all companies matching max_employees filter", async function () {
    ...
    test("throw error when min employees is bigger than max employees", async () => {
      await expect(
        Company.all({
          max_employees: 3,
          min_employees: 100,
        })
      ).rejects.toThrow("Min employees cannot be larger than max employees");
    });
  });

  describe("get()", () => {
    test("should get a company", async function () {
    ...
    test("should throw an error if company was not found", async () => {
      await expect(Company.get("KLJF")).rejects.toThrow("Company not found");
    });
  });
  ...
  describe("update()", () => {
    ...
    test("should throw an error if company was not found", async () => {
      ...
      await expect(Company.update("KLJF", valsToUpdate)).rejects.toThrow(
        "Company not found"
      );
    });
  });
  ...
  describe("delete()", () => {
    ...
    test("should throw an error if company was not found", async () => {
      await expect(Company.delete("KLJF")).rejects.toThrow("Company not found");
    });
  });
