...
describe("Query helper functions", function () {
  describe("searchQuery()", function () {
    test("should generate a proper query given one value", function () {
      const filters = {
        itemOne_colOne: "one",
      };
      ...
      expect(queryAndVals).toEqual({
        query:
          "SELECT colOne, numbers FROM test_table WHERE " + "colOne ILIKE $1",
        values: ["%one%"],
      });
    });

    test("should generate a proper query given multiple values", function () {
      const filters = {
        itemOne_colOne: "one",
        min_numbers: 100,
        max_numbers: 1000,
      ...
      expect(queryAndVals).toEqual({
        query:
          "SELECT colOne, numbers FROM test_table WHERE " +
          "colOne ILIKE $1 AND numbers >= $2 AND numbers <= $3",
        values: ["%one%", 100, 1000],
      });
    });

    test("should generate a proper query given no values", function () {
      const filters = {};
      const selectCols = ["colOne", "numbers"];
      let queryAndVals = searchQuery(filters, selectCols, "test_table");
      expect(queryAndVals).toEqual({
  ...
  describe("createValues()", function () {
    ...
    test("should generate a proper query given multiple values", function () {
      ...
      expect(queryAndVals).toEqual({
        query:
          "INSERT INTO test_table (colOne, colTwo, colThree) " +
          "VALUES ($1, $2, $3) RETURNING colOne",
        values: ["one", "two", "three"],
      });
    });

    test("should generate a bad query given no values", function () {
      const details = {};
      ...
      let queryAndVals = createValues(details, "test_table", allCols);
      expect(queryAndVals).toEqual({
        query: "INSERT INTO test_table () " + "VALUES () RETURNING ",
        values: [],
      });
    });
  });
  ...
  describe("partialUpdate()", function () {
    test("should generate a proper partial update query with just 1 field", function () {
      ...
      let queryAndVals = sqlForPartialUpdate(
        "companies",
        items,
        "handle",
        "TEST"
    ...
    test("should generate a proper full update with all fields", function () {
      const items = {
        handle: "ABC",
        name: "Alphabet",
        num_employees: 244,
    ...
    test("should generate a bad query with no fields", function () {
      const items = {};
